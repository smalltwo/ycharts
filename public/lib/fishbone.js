...
! function(global, DOC) {
	...
	*/
	...
	Data.init = function(key, value) {
		...
		if (value === undefined) {
			return this[0] && this[0]['d'] ? this[0]['d'][key] : null
	...
	Http.ajax = function(setting, events) {
		...
		// 将param转成post数据  20150821 发送post的数据格式的缘故，更改了parameter的赋值
		...
		req.setRequestHeader('Content-type', defaults.contentType)
			// req.setRequestHeader('Content-type', accepts.json)
	...
	Node.width = function() {
		...
		return parseInt(Css.init.call(this, 'width'))
	}

	// 获取元素的高
	Node.height = function() {

		return parseInt(Css.init.call(this, 'height'))
	}
	...
	Css.calculateChange = function(key, value) {
		...
		// 去掉px后取整
		oldValue = parseInt(oldValue.substring(0, oldValue.length - 2))
			// 去掉px后取整
	...
	Css.position = function() {
		...
		var top = parseInt(Css.init.call(this, 'top')),
			left = parseInt(Css.init.call(this, 'left')),
			right = parseInt(Css.init.call(this, 'right')),
			bottom = parseInt(Css.init.call(this, 'bottom'))

		top = top + parseInt(Css.init.call(this, 'margin-top'))
		left = left + parseInt(Css.init.call(this, 'margin-left'))
		right = right + parseInt(Css.init.call(this, 'margin-right'))
		bottom = bottom + parseInt(Css.init.call(this, 'margin-bottom'))
	...
	Css.offset = function() {
			...
			// offset.top += parseInt(Css.init.call(this, 'margin-top'))
			// offset.left += parseInt(Css.init.call(this, 'margin-left'))

			parentOffset.top += parseInt(Css.init.call(offsetParent, 'border-top-width'))
			parentOffset.left += parseInt(Css.init.call(offsetParent, 'border-left-width'))
	...
	Route.loadCss = function(arr) {
			...
			var callback = function() {
				...
				if (arr === undefined || (isArray && cssReady === arr.length) || !isArray) {
	...
	Animate.getBegins = function(params, keys) {
		...
		for (var i = 0; i < this.length; i++) {
			...
			for (var j = 0; j < keys.length; j++) {
				...
				params[key] = parseFloat(obj.css(key))
			}
	...
	Animate.getDistance = function(begin, end, duration, frame) {
		...
		for (var i = 0; i < begin.length; i++) {
			...
			for (var k in begin[i]) {
				...
				// 结束值 - 初始值 / 帧频
				changes.push((parseFloat(end[k]) - begin[i][k]) / (duration / frame))
			}
